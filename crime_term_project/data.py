...
def encode_labels(unordered_labels):
    """
    ...
    le.fit(unordered_labels)
    return le.transform(unordered_labels)
...
def main():
    df = load_data(
        'C:\\Users\\catic\\Documents\\EECE 2300\\python\\crime_term_project\\data\\raw\\communities.data.txt')
    # print (df)
    df2 = summarize_data(df)
    #print(df2)
    df_attributes = label_data(
        'C:\\Users\\catic\\Documents\\EECE 2300\\python\\crime_term_project\\data\\raw\\communities.attributes.txt',
        df2)
    # print(df_attributes)
